...
def minimax(board, depth, alpha, beta, bestaction, is_top, player, enemy):
    initial_eval = evaluate.evaluate(board)
    if ((initial_eval == -9999999) or (initial_eval == 9999999) or (initial_eval == -0.005) or (depth == 0)):
        return initial_eval
    val = evaluate.MINVAL
    if is_top:
        actions = generate_quick_best(board, player)
        for (a, _) in actions:
            temp = deepcopy(board)
            new_board = GamePlay.take_action(temp, a, player)
            val2 = maximin(new_board, depth-1, alpha, beta, bestaction, player, enemy)
            if (val2 > val):
                val = val2
                    ...
                    for box in range(1, 5):
                        for direction in ["R", "L"]:
                            ...
                            if isUnique(symmetricBoards(new_board), hashes_seen):
                                ...
                                val2 = maximin(
                                    new_board, depth-1, alpha, beta, bestaction, player, enemy)
                                if (val2 > val):
                                    val = val2
                                alpha = max(alpha, val)
                                if alpha >= beta:
...
def getBestAction(board, depth, player, enemy):

    bestaction = findAvailableAction(board)
    temp = deepcopy(board)
    minimax(temp, depth, evaluate.MINVAL, evaluate.MAXVAL, bestaction, True, player, enemy)
